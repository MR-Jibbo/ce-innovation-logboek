...
- **Mac**: omdat de app niet gesigneerd is met een Apple Developer-account (dat kost €99/jaar), krijgt een gebruiker bij de eerste keer openen een waarschuwing van macOS ("kan niet worden geopend omdat de ontwikkelaar niet geverifieerd is"). Dat los je op door in Finder rechts te klikken op de app → **Open** → **Open** bevestigen. Dit hoeft maar één keer.
- **Windows**: Windows SmartScreen kan een soortgelijke waarschuwing tonen ("Windows heeft je pc beschermd"). Klik op **Meer info** → **Toch uitvoeren**. Ook dit is eenmalig per installatie.

Dit is normaal voor kleine, niet-commerciële apps zonder betaald ontwikkelaarscertificaat en is geen teken dat er iets mis is.
